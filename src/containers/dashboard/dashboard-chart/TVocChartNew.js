...
import BarSeries from "../../../components/Charts/MyChart/BarSeries";
...
const TVocChartNew = React.memo(function TVocChartNew({data, width, height, sensorName, title, windSpeedData, setParentState, allDateValues, containerType}) {
    ...
    const [{
        showTooltip, tooltipKeys, x0_scaleBand, x1_scaleBand, chartType, tooltipLeft, tooltipTop, dimensions, originalWindBins, min, max, selected, updated, xScale, xScale2,
        yScale, chartData, originalData, chartBins, hoveredDate, hoveredValueState, yScale2, windData, windBins, hoveredDateString, legendData, showTooltipCircle, hoveredValueDetails
    }, setState] = useReducer(chartReducer, {
        x0_scaleBand: undefined,
        x1_scaleBand: undefined,
        xScale: undefined,
        xScale2: undefined,
        yScale: undefined,
        yScale2: undefined,
    ...
    function renderChart() {
        ...
        if (selectedDateRange && sensorName && data) {
            if (dimensions && dimensions.boundedWidth > 0) {
                ...
                let _xScale, _xScale2, _x0_scaleBand, _x1_scaleBand, _yScale, _yScale2;

                _xScale = d3.scaleTime().domain([
                    ...timeRange
                ]).rangeRound([0, dimensionsNew.boundedWidth]).nice();

                _x0_scaleBand = d3.scaleBand().padding(.2).domain(allDateValues).rangeRound([0, dimensionsNew.boundedWidth]);
                ...
                if (newData.length > 0) {
                    ...
                    _x0_scaleBand = d3.scaleBand().padding(.2).domain(...timeRange).rangeRound(_xScale.range());
                    ...
                    const chartBinGenerator =  d3.histogram()
                        .domain(_xScale.domain())
                        .value(function (d) {
                            return d.date;
                        })
                   ...
                   let newStackMap = {};
                    ...
                    data.map(item=> {
                        Object.keys(item.valueMap).forEach((valKey) => {
                            let valKeytAsDate = new Date(valKey);
                            if (!newStackMap[valKeytAsDate]) {
                                newStackMap[valKeytAsDate] = {date: valKeytAsDate};
                            }
                            newStackMap[valKeytAsDate][item.name] = item.valueMap[valKey][0]['tVOC1'];
                        });

                    });
                   //console.log(Object.values(newStackMap));

                   let datakeys = data.map(item=>item.name);
                   // console.log('newStackedData', newStackedData);
                    _x1_scaleBand = d3.scaleBand().padding(.1).domain( ...datakeys).rangeRound([0, _x0_scaleBand.bandwidth()]);

                    let stackedData = d3.stack()
                        .keys(datakeys)
                        .order(d3.stackOrderNone)
                        .offset(d3.stackOffsetNone)(Object.values(newStackMap));
                    ...
                    const _chartBins = [];
                    newData.map(item=>{
                        _chartBins.push({name: item.name, values: chartBinGenerator(item.values)});
                    });
                   // console.log('_x1_scaleBand, _x0_scaleBand', _x1_scaleBand, _x0_scaleBand)
                   // console.log('stackedData', Array.from(stackedData));
                    console.log('chartData', _chartBins);
                    //console.log('windSpeedData', windSpeedData);

                    window.dimensions = dimensions;
                    window.xScale = _xScale;
                    window.yScale= _yScale;
                    ...
                    setState({
                        ...
                        x0_scaleBand: _x0_scaleBand,
                        x1_scaleBand: _x1_scaleBand,
                        windBins: bins,
                        chartBins: _chartBins,
                        chartData: newData,
                ...
                <Chart dimensions={dimensions} style={{'border': '1px solid  green'}}>
                    ...
                    {chartType === 'barSeries' && (
                        <BarSeries
                            ...
                            xScale={xScale}
                            yScale={yScale}
                            x0_scaleBand={x0_scaleBand}
                            x1_scaleBand={x1_scaleBand}
                            dimensions={dimensions}
                            xAccessor={xAccessor}
                            yAccessor={yAccessor}
                        />
                        ...
                        <React.Fragment>
                                ...
                                <React.Fragment key={'frag_' + index}>
                                    {hoveredValueState[itemKey] && hoveredValueState[itemKey].val && (
                                        <div key={index} className={classes.tooltipItem}>
                                            ...
                                            <div style={{width: containerType && containerType === 'sites' && '180px'}}
                                                 className={classes.tooltipInnerItem}>{hoveredValueState[itemKey].label}:
                                            </div>
                                            <div className={classes.tooltipInnerItem}>
                                                <strong>{hoveredValueState[itemKey].val}</strong></div>
                                        </div>
                                    )}
                                </React.Fragment>
                            ))}
                            ...
                            && (<React.Fragment>
                                    <div key="Speed" className={classes.tooltipItem}>
                                        ...
                                        <div className={classes.tooltipInnerItem}>
                                            <strong>{hoveredValueState['Speed'].val}</strong></div>
                                    </div>
                                </React.Fragment>
                            )}
                                ...
                                <React.Fragment>
                                    <div key="Direction" className={classes.tooltipItem}>
                                        ...
                                        <div style={{paddingLeft: '3px'}}
                                             className={classes.tooltipInnerItem}>Direction:
                                        </div>
                                        <div className={classes.tooltipInnerItem}>
