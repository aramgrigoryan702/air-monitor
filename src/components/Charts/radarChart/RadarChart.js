...
function drawBars(ref) {


const dateParser = d3.timeParse("%Y-%m-%d")
const dateAccessor = d => dateParser(d.date)
const data = DataSet.sort((a,b) => dateAccessor(a) - dateAccessor(b))
const metrics = [
  "windBearing",
  "moonPhase",
  "pressure",
...
const updateChart = () => {
  title.text(dateFormatter(dateAccessor(data[activeDayIndex])))
  drawLine(data[activeDayIndex])
}

updateChart()

d3.select("#show-next-day").on("click", e => {
  activeDayIndex = (activeDayIndex + 1) % (data.length - 1)
  updateChart()
})
}
